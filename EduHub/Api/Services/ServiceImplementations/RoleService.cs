...
{
    public async Task<long> AddUserAsync(RoleCreateDto roleCreateDto, string roleName)
    {
        if (roleName != "SuperAdmin")
            throw new NotAllowedException("Only super admin can add roles");
    ...
    public async Task DeleteRoleAsync(string deletingRole, string roleName)
    {
        if (roleName != "SuperAdmin")
            throw new NotAllowedException("Access denied for non SuperAdmin");
    ...
    //public async Task<IEnumerable<RoleDto>> GetAllUsersByRolesNameAsync(string roleName)
    //{
    //    var users = await roleRepository.SelectAllUsersByRoleNameAsync(roleName);
    //    return users.Select(RoleMapper.ToRoleDto).
    //}
